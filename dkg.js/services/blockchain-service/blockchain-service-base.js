...
const IdFactoryAbi = require('dkg-evm-module/abi/IdFactory.json');
const IdentityAbi = require('dkg-evm-module/abi/Identity.json');
const ClaimIssuerAbi = require('dkg-evm-module/abi/ClaimIssuer.json');
...
class BlockchainServiceBase {
    constructor(config = {}) {
        ...
        this.abis.TREXFactory = TREXFactoryAbi;
        this.abis.Context = ContextAbi;
        this.abis.IdFactory = IdFactoryAbi;
        this.abis.Identity = IdentityAbi;
        this.abis.ClaimIssuer = ClaimIssuerAbi;

        this.abis.ClaimTopicsRegistry = ClaimTopicsRegistryAbi;
        this.abis.TrustedIssuersRegistry = TrustedIssuersRegistryAbi;
    ...
    async getContextAddress(context, blockchain) {
        const contextAddress = await this.callContractFunction(
            'TREXFactory',
            'getContext',
            [context],
            blockchain
    ...
    async getWeb3Provider(blockchain) {
        if (!this[blockchain.name].web3) {
            const blockchainOptions = {
                transactionPollingTimeout: blockchain.transactionPollingTimeout,
            };
    ...
    async callContractFunction(contractName, functionName, args, blockchain, contractAddress = null) {
        await this.ensureBlockchainInfo(blockchain);
        ...
        try {
            ...
            else if (/This contract object doesn't have address set yet/i.test(error.message)) {
                error.message += ` ${  contractName}`;
            }
            console.log(error);
            throw error;
        }
    ...
    async prepareTransaction(contractInstance, functionName, args, blockchain) {
        ...
        if (blockchain.previousTxGasPrice && blockchain.retryTx) {
            // Increase previous tx gas price by 50%
            gasPrice = Math.round(blockchain.previousTxGasPrice * 1.5);
            ...
            if (currentNonce > confirmedNonce) {
                ...
                if (pendingTx) {
                    // If found, increase gas price of pending tx by 50%
                    gasPrice = Math.round(Number(pendingTx.gasPrice) * 1.5);
                } else {
                    // If not found, use default/network gas price increased by 50%
    ...
    async getContractInstanceWithAddress(contractName, blockchain, contractAddress) {
        const web3Instance = await this.getWeb3Instance(blockchain);
