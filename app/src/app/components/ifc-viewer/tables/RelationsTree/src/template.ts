...
import {IfcElements} from "@thatopen/components/dist/ifc/Utils/ifc-elements-map";
...
const getDecompositionTree = async (
  ...
  const entityRow: BUI.TableGroupData = {
    data: {
      ...
      modelID: model.uuid,
      expressID,
      globalId: entityAttrs['GlobalId']?.value
    },
  };
...
export const relationsTreeTemplate = (state: RelationsTreeUIState) => {
  ...
  if (!table) {
    ...
    table.hiddenColumns = ["relations", "modelID", "globalId"];
    table.columns = ["Entity", "Name", "expressID"];
