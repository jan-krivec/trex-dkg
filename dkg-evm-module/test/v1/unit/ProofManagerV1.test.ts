...
describe('@v1 @unit ProofManagerV1 contract', function () {
  ...
  it('Each node submits commits and proofs for each epoch, verify all rewards in all epochs are the same, and total rewards equals initial context amount in service agreement', async () => {
    const operationalWallets = [accounts[0], accounts[2], accounts[3]];
    const identities = [];
  ...
  it('Variable number of nodes send commits and proofs each epoch, verify all rewards in all epochs are the same, and total rewards equals initial context amount in service agreement', async () => {
