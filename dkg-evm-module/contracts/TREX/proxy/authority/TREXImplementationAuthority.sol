...
contract TREXImplementationAuthority is ITREXImplementationAuthority, Ownable {
    ...
    function fetchVersion(Version calldata _version) external override {
        ...
        if (_contracts[_versionToBytes(_version)].contextImplementation != address(0)) {
    ...
    function changeImplementationAuthority(address _context, address _newImplementationAuthority) external override {
        ...
        address _ir = address(IContext(_context).identityRegistry());
        address _mc = address(IContext(_context).compliance());
        ...
        if(
            Ownable(_context).owner() != msg.sender
        ...
        if(_newImplementationAuthority == address(0)) {
            _newImplementationAuthority = IIAFactory(_iaFactory).deployIA(_context);
        }
        else {
            if(
        ...
        IProxy(_context).setImplementationAuthority(_newImplementationAuthority);
        ...
        emit ImplementationAuthorityChanged(_context, _newImplementationAuthority);
    }
    ...
    function getContextImplementation() external view override returns (address) {
        return _contracts[_versionToBytes(_currentVersion)].contextImplementation;
    }
    ...
    function addTREXVersion(Version calldata _version, TREXContracts calldata _trex) public override onlyOwner {
        ...
        require(
            ...
            && _trex.contextImplementation != address(0)
