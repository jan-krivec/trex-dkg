...
contract TrustedIssuersRegistry is ITrustedIssuersRegistry, HubDependentInitializable, TIRStorage {
     ...
     */
    function addTrustedIssuer(address _trustedIssuerAdr, uint256[] calldata _claimTopics) external override {
        addTrustedIssuerInternal(msg.sender, _trustedIssuerAdr, _claimTopics);
    }

    function addTrustedIssuer(address sender, address _trustedIssuerAdr, uint256[] calldata _claimTopics) external override {
        addTrustedIssuerInternal(sender, _trustedIssuerAdr, _claimTopics);
    }

    function addTrustedIssuerInternal(address sender, address _trustedIssuerAdr, uint256[] calldata _claimTopics) internal {
        checkOnlyAgent(sender);
        require(_trustedIssuerAdr != address(0), "invalid argument - zero address");
        require(_trustedIssuerClaimTopics[_trustedIssuerAdr].length == 0, "trusted Issuer already exists");
        ...
        IClaimIssuer _trustedIssuer = IClaimIssuer(_trustedIssuerAdr);
        _trustedIssuers.push(_trustedIssuer);
        _trustedIssuerClaimTopics[_trustedIssuerAdr] = _claimTopics;
     ...
     */
    function removeTrustedIssuer(address _trustedIssuerAdr) external override onlyHubAgent(msg.sender) {
        require(_trustedIssuerAdr != address(0), "invalid argument - zero address");
        ...
        for (
            ...
            claimTopicIndex < _trustedIssuerClaimTopics[_trustedIssuerAdr].length;
            claimTopicIndex++) {
            uint256 claimTopic = _trustedIssuerClaimTopics[_trustedIssuerAdr][claimTopicIndex];
        ...
        delete _trustedIssuerClaimTopics[_trustedIssuerAdr];
    ...
    function updateIssuerClaimTopics(address _trustedIssuerAdr, uint256[] calldata _claimTopics) external override onlyHubAgent(msg.sender) {
        ...
        for (uint256 i = 0; i < _trustedIssuerClaimTopics[_trustedIssuerAdr].length; i++) {
            uint256 claimTopic = _trustedIssuerClaimTopics[_trustedIssuerAdr][i];
            uint256 topicsLength = _claimTopicsToTrustedIssuers[claimTopic].length;
