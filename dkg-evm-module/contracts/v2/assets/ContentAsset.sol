...
contract ContentAssetV2 is Named, Versioned, HubDependentV2, DkgInitializable {
    ...
    function createAsset(ContentAssetStructs.AssetInputArgs calldata args) onlyVerifiedContext(args.contexts) external returns (uint256) {
        return
            _createAsset(
                msg.sender,
                args.assertionId,
    ...
    ) external onlyVerifiedContext(args.contexts) onlyContracts returns (uint256) {
        return
            _createAsset(
                originalSender,
                args.assertionId,
    ...
    function createAssetWithVariables(
        ...
        bool immutable_
    ) onlyVerifiedContext(contexts) external returns (uint256) {
        return
            _createAsset(
                msg.sender,
                assertionId,
    ...
    function cancelAssetStateUpdate(uint256 tokenId) external onlyAssetOwner(tokenId) {
        _cancelAssetStateUpdate(tokenId);
    }

    function cancelAssetStateUpdateFromContract(uint256 tokenId) external onlyContracts {
        _cancelAssetStateUpdate(tokenId);
    }
    ...
    function _cancelAssetStateUpdate(uint256 tokenId) internal virtual {
        ContentAssetStorage cas = contentAssetStorage;
        ServiceAgreementStorageProxy sasProxy = serviceAgreementStorageProxy;
        UnfinalizedStateStorage uss = unfinalizedStateStorage;
