...
interface IIdFactory {
    ...
    // event emitted when a asset is linked to an ONCHAINID contract
    event AssetLinked(address indexed asset, address indexed identity);
     ...
     *  @param _assetOwner the owner address of the asset
     *  @param _salt the salt used by create2 to issue the contract
     *  requires a new salt for each deployment
     *  _asset cannot be linked to another ONCHAINID
     *  only Token factory or owner can call (owner should only use its privilege
     *  for tokens not issued by a Token factory onchain
     */
    function createAssetIdentity(address _asset, address _assetOwner, string memory _salt) external returns (address);
